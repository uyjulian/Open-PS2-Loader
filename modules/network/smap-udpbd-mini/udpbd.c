...
int udpbd_init(void)
{
    ...
    // Wait for a connection (interupt will jump out of this loop)
    while (!bdm_connected) {}
    ...
    switch (hdr.cmd) {
        ...
        case UDPBD_CMD_READ:
            if ((g_buffer != NULL) && (g_read_size >= hdr.par1) && (g_cmdid == hdr.cmdid)) {
                ...
                UDPBDCopyFromFIFOWithDMA(smap_regbase, (u8 *)g_buffer + ((g_read_cmdpkt - 2) * UDPBD_MAX_DATA), (hdr.par1 / 128));
